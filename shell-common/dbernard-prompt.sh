# dbernard@localhost [~/path/to/somewhere] [version-control-status] -------------------------------------------- [something?]
...
function _dbernard_prompt_virtualenv() {
...
function _dbernard_prompt_title() {
...
function _dbernard_filter_ansi() {
...
function _dbernard_prompt() {
    ...
    virtualenv_status=$(_dbernard_prompt_virtualenv)
    ...
    topline=$(_dbernard_filter_ansi "$topline")
    ...
    current_dir="${fg_light_yellow}[${fg_light_magenta}${current_dir}${fg_light_yellow}]${ansi_reset}"
...
DBERNARD_PROMPT_PS1="${fg_light_blue}::${ansi_reset} "
